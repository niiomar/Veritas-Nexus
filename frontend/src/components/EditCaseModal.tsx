...
export const EditCaseModal: React.FC<EditCaseModalProps> = ({ initialCase, onClose, onSubmit }) => {
  const [alias, setAlias] = useState(initialCase.alias);
  const [name, setName] = useState(initialCase.name);
  const [priority, setPriority] = useState(initialCase.priority || 'LOW');
  const [analyst, setAnalyst] = useState(initialCase.analyst);
      ...
      <div style={{ backgroundColor: '#0a0a0c', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', width: '400px', padding: '32px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '16px' }}>
            ...
            <button type="submit" className="mono hover-bright" style={{ padding: '8px 16px', background: 'var(--text-main)', border: 'none', color: '#000', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 600 }}>UPDATE CASE</button>
          </div>
        </form>
      </div>
    </div>
