...
def verify_c2pa_provenance(file_path: str) -> dict:
    ...
    try:
        ...
        # 1. Determine if the file is signed based on the ProvenanceReport status
        ...
        # 2. Extract the raw JSON store to build the detailed graph
        ...
        # 3. Extract Cryptographic specifics
        ...
        # 4. Extract Software Agent / Generator
        ...
        # 5. Build the Provenance Graph Timeline mapping perfectly to the frontend
        history = []
        
        # Step A: Parse embedded internal actions (Origin, Converted, etc.)
        for assertion in active_mdata.get("assertions", []):
            if "c2pa.actions" in assertion.get("label", ""):
                for act in assertion.get("data", {}).get("actions", []):
                    ...
                    # Normalize "c2pa.created" to "Created"
        ...
        # Step B: Append the Cryptographic Signature node
        history.append({
            "action": "Signed",
            "agent": issuer,
            "timestamp": timestamp,
        ...
        # Step C: Append the Verification node
        history.append({
            "action": "Verified",
            "agent": "C2PA Veritas",
            "timestamp": "Present",
...
async def execute_correlation_engine(job_id: str, evidence_id: str, session):
    ...
    if real_probability is None:
        if c2pa_data["is_signed"] and c2pa_data["status"] == "VALID":
            ...
            disposition = "CONFLICT - Cryptographic signature present but partial/invalid."
            status_flag = "CONFLICT"
        else:
            disposition = "UNKNOWN - No signature found and Neural Engine offline."
            status_flag = "UNKNOWN"
    else:
        if real_probability < 0.15:
            if c2pa_data["is_signed"] and c2pa_data["status"] == "VALID":
                ...
                disposition = "HIGH TRUST - No synthetic artifacts, but lacks cryptographic signature."
                status_flag = "VERIFIED"
